...
function ensureDataFile() {
  ...
  const data = readData();
  if (!Array.isArray(data.cocktails) || !Array.isArray(data.notes)) {
    writeData(initialData);
    return initialData;
  }
...
app.post("/api/cocktails", (req, res) => {
  ...
  const newCocktail = {
    ...
    image:
      image ||
      "https://www.thecocktaildb.com/images/media/drink/rvwrqq1472817326.jpg",
    instructions,
...
app.get("/api/cocktails/:cocktailId/notes", (req, res) => {
  const notes = data.notes.filter((note) => note.cocktailId === req.params.cocktailId);
  res.json(notes);
});

app.post("/api/cocktails/:cocktailId/notes", (req, res) => {
  const { text } = req.body;
  if (!text) {
